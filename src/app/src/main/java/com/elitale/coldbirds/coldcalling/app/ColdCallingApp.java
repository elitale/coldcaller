 ...
 *
 * <p>Responsibility: launch the JavaFX runtime, perform dependency-injection wiring in {@link
 * #init()}, and hand the primary {@link Stage} to {@link MainWindow}. No business logic lives here.
 *
 * <p>All credentials (Twilio Account SID + Auth Token, SIP credentials) are read from the SQLite
 * {@code settings} table via {@link SettingsService}. On first run they are blank and the user
 * configures them through the {@link OnboardingWindow} setup wizard, after which {@link
 * #launchMainApp(Stage)} builds the credential-dependent services. The SIP stack and provider
 * clients degrade gracefully when credentials are absent.
 */
public final class ColdCallingApp extends Application {
  ...
  /**
   * Called on the JavaFX launcher thread before {@link #start}. Performs credential-independent
   * wiring: DB open, Flyway migration, repositories, settings, audio enumeration, and the
   * onboarding service. Credential-dependent services (provider clients, telephony) are built later
   * in {@link #launchMainApp}.
   */
  @Override
  public void init() {
    ...
    try {
      ...
      final PhoneNumberService onboardingNumbers =
          new PhoneNumberService(
              phoneNumberRepo, new TwilioClient(TwilioConfig.of("", "")), settingsRepo);
      onboardingService =
          new OnboardingService(settingsService, onboardingNumbers, new SipTester());
  ...
  /**
   * Build and start the credential-dependent services from the now-persisted settings, then show
   * the main window. Invoked either directly (when onboarding was already completed) or as the
   * onboarding wizard's completion callback. Must run on the FX Application Thread.
   */
  private void launchMainApp(Stage stage) {
    // Providers — credentials from settings table (written by onboarding or Settings).
    final TwilioClient twilio =
        new TwilioClient(
            TwilioConfig.of(
    ...
    callService =
        new CallService(telephonyService, callRepo, leadRepo, phoneNumberRepo, settingsService);
    ...
    telephonyService.setCountryResolver(number -> CountryLookup.byE164(CountryCatalog.ALL, number));
    ...
    mainWindow.setOnCallLogAutoSave(
        (disposition, notes) -> {
    ...
    smsService.startReceiving(
        sms ->
            mainWindow.onInboundSms(
                inboundLabel(sms.from()), sms.occurredAt().isAfter(smsWatchSince)));
  ...
  /**
   * Build the dismiss action for the failed/cancelled calling screen: stop timers/tones and return
   * to the screen the call was started from.
   */
  ...
  /**
   * Finalise a call from the wrap-up screen: persist the disposition and notes the rep entered
   * after the line dropped, then return to the originating screen.
   */
  ...
  /**
   * Finalise a call that ended via a voicemail drop during a power-dialer run: record the Voicemail
   * disposition, return to the dialer, and advance to the next lead — no wrap-up screen. Runs on
   * the call-ended (JAIN-SIP) thread; the call record is already persisted by the time this fires.
   */
  ...
  /**
   * Callback the Settings screen invokes when the user saves audio devices: resolves the persisted
   * device ids to mixers and applies them to telephony for the next call.
   */
  ...
  /**
   * Callback the in-call audio menu invokes to switch devices mid-call: resolves the ids and
   * applies them to the live call on a background thread (opening audio lines blocks the caller),
   * then writes them through to settings so they persist for future calls.
   */
  ...
  /**
   * No-op listener used only during TelephonyService construction before CallService has been built
   * and wired back via {@code setListener()}.
   */
  private static final TelephonyService.TelephonyListener NOOP_LISTENER =
      new TelephonyService.TelephonyListener() {
        @Override
        public void onIncomingCall(String id, PhoneNumber c, PhoneNumber d) {}

        @Override
        public void onCallAnswered(String id) {}

        @Override
        public void onCallEnded(String id, String r) {}

        @Override
        public void onRegistrationChanged(boolean reg) {}
      };

  /**
   * Returns the remote party E.164 number for an active call, falling back to the raw callId if the
   * call cannot be found (should not happen in practice).
   */
