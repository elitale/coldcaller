...
/**
 * Locks the {@link PowerDialerSession} invariants. This record is rebuilt on every UI tick
 * from the live dialer state, so any violation throws on the FX thread and crashes the app —
 * the constructor is the last line of defence and must reject every illegal combination.
 */
class PowerDialerSessionTest {
    ...
    private static PowerDialerSession session(int dialedCount, int connectedCount) {
        return new PowerDialerSession(0L, LIST, 0, new PowerDialerState.Running(),
                dialedCount, connectedCount, NOW, Optional.empty());
    }
    ...
    void rejectsNegativePosition() {
        assertThatThrownBy(() -> new PowerDialerSession(0L, LIST, -1, new PowerDialerState.Running(),
                0, 0, NOW, Optional.empty()))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void rejectsNullCallListId() {
        assertThatThrownBy(() -> new PowerDialerSession(0L, null, 0, new PowerDialerState.Running(),
                0, 0, NOW, Optional.empty()))
    ...
    void rejectsNullState() {
        assertThatThrownBy(() -> new PowerDialerSession(0L, LIST, 0, null,
                0, 0, NOW, Optional.empty()))
                .isInstanceOf(NullPointerException.class)
    ...
    void rejectsNullStartedAt() {
        assertThatThrownBy(() -> new PowerDialerSession(0L, LIST, 0, new PowerDialerState.Running(),
                0, 0, null, Optional.empty()))
                .isInstanceOf(NullPointerException.class)
    ...
    void buildsValidSession() {
        PowerDialerSession s = new PowerDialerSession(7L, LIST, 4, new PowerDialerState.Paused(),
                10, 4, NOW, Optional.of(NOW));
