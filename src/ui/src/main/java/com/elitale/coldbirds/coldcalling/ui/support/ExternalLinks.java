...
/**
 * App-wide bridge for opening external links (web pages, {@code mailto:}) in the user's default
 * browser/mail client.
 *
 * <p>JavaFX exposes this capability only through {@link javafx.application.HostServices}, which is
 * reachable from the {@code Application} instance and nowhere else. Rather than thread that handle
 * through every constructor down to leaf views, the app wires it once at startup via {@link
 * #configure(Consumer)} and leaf nodes call {@link #open}/{@link #openMail}. Using {@code
 * HostServices} (instead of {@code java.awt.Desktop}) avoids initialising AWT alongside the JavaFX
